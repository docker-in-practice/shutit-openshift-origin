...
class unduly(ShutItModule):
	...
	def build(self, shutit):
		...
		shutit.add_to_bashrc('export CURL_CA_BUNDLE=`pwd`/openshift.local.config/master/ca.crt')
		shutit.send('export CURL_CA_BUNDLE=`pwd`/openshift.local.config/master/ca.crt')
		shutit.send('sleep 30')
		shutit.send('oadm registry --create --credentials=/var/lib/openshift/openshift.local.config/master/openshift-registry.kubeconfig --config=/var/lib/openshift/openshift.local.config/master/admin.kubeconfig')
		shutit.send_until('''osadm registry --create --credentials=/var/lib/openshift/openshift.local.config/master/openshift-registry.kubeconfig --images='registry.access.redhat.com/openshift3_beta/ose-${component}:${version}' --selector="region=infra" --mount-host=/mnt/registry''','.*registry.*')
		shutit.send('oc describe service docker-registry --config=openshift.local.config/master/admin.kubeconfig')
		shutit.send('osadm policy add-role-to-user view test-admin --config=openshift.local.config/master/admin.kubeconfig')
		shutit.multisend('oc login --certificate-authority=openshift.local.config/master/ca.crt',{'assword':'any','sername':'test-admin'})
		shutit.send('oc new-project test --display-name="OpenShift 3 Sample" --description="This is an example project to demonstrate OpenShift v3"')
		shutit.send('oc create -f <(curl -k https://raw.githubusercontent.com/openshift/origin/master/examples/image-streams/image-streams-centos7.json) -n test')
		shutit.send('oc project default')
		service = shutit.send_and_get_output("""oc get service | awk '{print $4 $5}'""")
		...
		shutit.log('Run:\n\ndocker exec -it openshift-origin bash\n\nto see the container',add_final_message=True)
		shutit.log('Access https://localhost:8443 for the GUI, log in as test-admin',add_final_message=True)
		return True
